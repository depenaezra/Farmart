                        ...
                        <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer bg-gray-50">
                                                <?php if ($violation['item_exists']): ?>
                                                    ...
                                                    <?php if (in_array($violation['reported_type'], ['forum_post', 'forum_comment'], true) && !empty(($violation['reported_item']['is_suspended'] ?? null))): ?>
                                                        <form action="/admin/moderation/restore" method="POST" class="me-auto swal-confirm-form" data-confirm-title="Restore this content?" data-confirm="The item will become visible again." data-confirm-ok="Restore" data-confirm-icon="question">
                                                            <?= csrf_field() ?>
                                                            <input type="hidden" name="type" value="<?= esc($violation['reported_type']) ?>">
                                                            <input type="hidden" name="id" value="<?= (int) $violation['reported_id'] ?>">
                            ...
                            <div class="lg:w-64 flex flex-col gap-3">
                                <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                                    ...
                                    <?php
                                    $reportedUserId = null;
                                    if (!empty($violation['reported_item']['user_id'])) {
                                        $reportedUserId = (int) $violation['reported_item']['user_id'];
                                    } elseif ($violation['reported_type'] === 'user') {
                                        $reportedUserId = (int) $violation['reported_id'];
                                    }
                                    ?>

                                    <?php if ($reportedUserId): ?>
                                        <form action="/admin/moderation/block-user" method="POST" class="swal-confirm-form" data-confirm-title="Block this user?" data-confirm="The user's email will be blocked and the account disabled." data-confirm-ok="Block user" data-confirm-danger data-confirm-icon="warning">
                                            <?= csrf_field() ?>
                                            <input type="hidden" name="user_id" value="<?= (int) $reportedUserId ?>">
                                            <input type="hidden" name="reason" value="blocked_from_violation_<?= esc($violation['reported_type']) ?>">
                                            <button type="submit" class="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-rose-50 text-rose-700 border border-rose-200 rounded-lg hover:bg-rose-100 transition-colors text-sm font-semibold mt-2">
