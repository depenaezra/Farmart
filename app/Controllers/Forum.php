...
class Forum extends BaseController
{
    ...
    public function index()
    {
        ...
        // Hide suspended posts for the public.
        if (session()->get('user_role') !== 'admin') {
            $query->where('forum_posts.is_suspended', 0);
        }
    ...
    public function viewPost($id)
    {
        ...
        // If suspended: only author or admin can view.
        if (!empty($post['is_suspended'])) {
            $isAdmin = session()->get('user_role') === 'admin';
            $isOwner = (int) ($post['user_id'] ?? 0) === (int) session()->get('user_id');
            if (!$isAdmin && !$isOwner) {
                return redirect()->to('/forum')
                    ->with('error', 'This post is under review.');
            }
        }
        ...
        $commentsQuery = $db->table('forum_comments')
                       ->select('forum_comments.*, users.name as author_name, users.role as author_role')
                       ->join('users', 'users.id = forum_comments.user_id')
        ...
        if (session()->get('user_role') !== 'admin') {
            // hide suspended comments for non-admins
            try {
                $commentsQuery->where('forum_comments.is_suspended', 0);
            } catch (\Exception $e) {
                // ignore if column doesn't exist yet
            }
        }

        $comments = $commentsQuery->get()->getResultArray();
