...
class Admin extends BaseController
{
    ...
    public function users()
    {
        ...
        $data = [
            ...
            'current_role' => $role,
            'search_term' => $search,
            'email_whitelist_enabled' => $this->systemSettingModel->isEnabled('email_whitelist_enabled', false),
            'whitelisted_emails' => $this->whitelistedEmailModel->listEmails(),
        ];
    ...
    /**
     * Update email whitelist from User Management page.
     * When enabled, automatically disables all NON-admin accounts that are not whitelisted.
     */
    public function updateWhitelist()
    {
        $enabled = $this->request->getPost('email_whitelist_enabled') ? true : false;
        $rawEmails = (string) $this->request->getPost('whitelisted_emails');
        ...
        $this->systemSettingModel->setValue('email_whitelist_enabled', $enabled ? '1' : '0');
        $this->whitelistedEmailModel->replaceAll($cleanList, (int) session()->get('user_id'));

        if ($enabled) {
            // Disable all non-admins not in whitelist.
            $db = \Config\Database::connect();
            $builder = $db->table('users')->where('role !=', 'admin');
            if (!empty($cleanList)) {
                $builder->whereNotIn('email', $cleanList);
            }
            $builder->update(['status' => 'inactive']);
        }

        return redirect()->to('/admin/users')
            ->with('success', 'Whitelist settings updated.');
    }
    ...
    /**
     * Restore a suspended forum post/comment (moderation undo).
     */
    public function restoreSuspendedContent()
    {
        $type = (string) $this->request->getPost('type');
        $id = (int) $this->request->getPost('id');

        if (!in_array($type, ['forum_post', 'forum_comment'], true) || $id <= 0) {
            return redirect()->back()->with('error', 'Invalid restore request.');
        }

        $db = \Config\Database::connect();
        $table = $type === 'forum_post' ? 'forum_posts' : 'forum_comments';
        ...
        try {
            $db->table($table)->where('id', $id)->update([
                ...
                'suspended_by' => (int) session()->get('user_id'),
            ]);
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Unable to restore content.');
        }

        return redirect()->back()->with('success', 'Content restored successfully.');
    }

    /**
     * Block a user (via email blocker) quickly from moderation screens.
     */
    public function blockUserFromModeration()
    {
        ...
        if (!$this->blockedEmailModel->isBlocked($email)) {
            $this->blockedEmailModel->blockEmail($email, (int) session()->get('user_id'), $reason !== '' ? $reason : 'blocked_from_moderation');
        }

        // Disable account status too.
        $this->userModel->update($userId, ['status' => 'inactive']);

        return redirect()->back()->with('success', 'User blocked and account disabled.');
    }
