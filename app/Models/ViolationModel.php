            ...
            switch ($violation['reported_type']) {
                case 'forum_post':
                    $violation['reported_item'] = $db->table('forum_posts')
                        ->select('content, user_id, is_suspended, suspended_at, suspended_reason')
                    ...
                    break;
                case 'forum_comment':
                    $violation['reported_item'] = $db->table('forum_comments')
                        ->select('comment, user_id, is_suspended, suspended_at, suspended_reason')
                        ->where('id', $violation['reported_id'])
                        ->get()
                        ->getRowArray();
                    $violation['item_exists'] = !empty($violation['reported_item']);
